...
rpoisball.unit <- function(n,d){
  ...
  r1 <- runif(n,0,1)^(1/d)
  norm.data <- mvrnorm(n,rep(0,d),diag(d))
...
# simulate a homogenuous poisson process in a sphere of some radius and center
# requires library MASS
rpoisball.scaled <- function(n,d,r,c){
  ...
  # fix for a radius and a center
  # data1 <- c + data1*r
  data1 <- t(apply(data1, 1, function(x) c + x*r))
...
# requires library MASS
rpoisbox.unit <- function(n,d, min = 0, max = 1){
  
  # if a single min or max are entered, replicate given the number of dimensions
  if(length(min) == 1)
    min <- rep(min, d)
  if(length(max) == 1)
    max <- rep(max, d)
  ...
  data1 <- mapply(function(x,y,z) runif(n,y,z), 1:d, min, max)
...
# simulate Box Matern process in a unit box
# all points are in the window, it is a special kind of matern process
# version that cluster centers are stored
# nclus: number of clusters
# mu: # of obs in cluster
# r: radius of cluster
# d: dimension
rMaternUnif.withcenters <- function(nclus,mu,r,rperc,d,dist.method="maximum"){
  ...
  return(list(data = datax, centers = dataclus))
}
...
# mu: # of obs in cluster
# r: radius of cluster
# rperc: the percentage of the radius that is defined to be the minimum distance between two class borders 
# d: dimension
# dist.method: the distance method, 
rMaternNormal <- function(nclus,mu,r,rperc,d,dist.method="euclidean"){
